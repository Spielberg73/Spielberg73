...
class TestEditorConVerbos(unittest.TestCase):
    """El editor con la aventura grafica, donde todas las casillas son 'vacio'.

    Ahi el tipo no distingue nada -un cursor no choca con nada- y lo unico que
    separa un cuadro de la pared de al lado es a que verbo contesta. Ademas se
    comprueba lo que mas duele en este genero: que guardar el yaml despues de
    pintar no se lleve por delante los verbos de la leyenda."""
    ...
    def setUpClass(cls):
        if not shutil.which("node"):
            raise unittest.SkipTest("node no esta instalado")
        from ngplat.scaffold import crear_proyecto
        cls.tmp = tempfile.mkdtemp(prefix="neoplat-editor-grafica-")
        proyecto = os.path.join(cls.tmp, "grafica")
        crear_proyecto(proyecto, "LA CASA", "TEST", genero="grafica")
        datos = build_data(build_project(load_project(proyecto)))
        for hoja in datos["sheets"].values():
            hoja["url"] = ""
        cls.datos = os.path.join(cls.tmp, "datos.json")
    ...
    def test_el_editor_conoce_los_verbos(self):
        resultado = subprocess.run(
            ["node", os.path.join(KIT, "tests", "editor_grafica.js"), self.datos],
            capture_output=True, text=True,
        )
        self.assertEqual(resultado.returncode, 0, resultado.stdout + resultado.stderr)
