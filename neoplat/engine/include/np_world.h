...
typedef struct {
    ...
    uint8_t trepa;
    /* La liana de balanceo, en **un solo byte** y con signo:
     *
     *    > 0  de esa liana vas colgado: es su sitio en la lista mas uno. No
     *         es `trepa`: de aquella subes, de esta te dejas llevar y lo unico
     *         que decides es cuando soltarte;
     *    < 0  frames que quedan sin poder agarrarte a ninguna. Sin esto
     *         soltarse no sirve de nada: te sueltas en la punta y al frame
     *         siguiente sigues a un palmo de ella, asi que te vuelves a
     *         enganchar y el que mira ve a alguien pegado a la liana para
     *         siempre;
     *      0  ni una cosa ni la otra.
     *
     * Las dos cosas caben en un byte porque nunca pasan a la vez, y **tienen**
     * que caber: aqui hay un hueco de un byte y ni uno mas. Con dos campos la
     * estructura del jugador pasa de 72 bytes a 74, gcc cambia de estrategia y
     * el juego de ejemplo le cuesta a la Neo Geo 2000 ciclos mas por frame -de
     * los 200000 que da- con cocodrilos o sin ellos. Medido: 198746 con un
     * byte y 200758 con dos, y da igual lo que guarden: con dos bytes de
     * relleno que no lee nadie sale el mismo numero. */
    int8_t balanceo;
