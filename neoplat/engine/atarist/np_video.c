...
void np_video_actores(const NpWorld *w)
{
    ...
    /* Lo que hay en la calzada va **donde dice la proyeccion y del tamano que
       ...
       El motor da la pantalla ya proyectada, y aqui se pasa a coordenadas de
       mundo porque es lo que pide el pintor. */
    /* De mas lejos a mas cerca, y ese orden lo da el motor: la lista de
       entidades no sirve, que aqui lo que manda es la escala. */
    {
        ...
        for (j = 0; j < cuantos; j++) {
            ...
            np_pintar_bloque(b, tam->first_tile, tam->cols, tam->rows,
                             visto[j].sx - tam->cols * NP_TILE / 2 + b->vista_x,
                             visto[j].sy - tam->rows * NP_TILE + b->vista_y
                                 - NP_RECORTE_Y,
