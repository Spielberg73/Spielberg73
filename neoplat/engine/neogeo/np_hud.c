...
/* Aqui el marcador vive arriba del todo (filas 1 y 2) y los mensajes en medio
   de la pantalla, asi que el cuadro de texto de los guiones va donde ya sale
   "LEVEL CLEAR" y no debajo del tanteo. Va **antes** de los includes: np_types.h
   lo pone a 1 si no se lo ha dicho nadie. */
#define NP_DIALOGO_FILA 12
     ...
     * siguiente. */
    {
        ...
        if (hay != np_hud_cuadro) {
            ...
            np_hud_print(NP_DIALOGO_COL, NP_DIALOGO_FILA,
                         np_dialogo_linea(w, 0), NP_HUD_PALETTE);
            np_hud_print(NP_DIALOGO_COL, NP_DIALOGO_FILA + 1,
                         np_dialogo_linea(w, 1), NP_HUD_PALETTE);
