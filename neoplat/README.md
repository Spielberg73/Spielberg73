...
| | |
|---|---|
| `neoplat-docs-1.15.zip` | sólo la documentación: este README y todo `docs/`. Es lo que te llevas si quieres leerla o pasársela a otro proyecto |
| `neoplat-kit-1.15.zip` | el kit entero: motor, herramientas, ejemplo y pruebas, sin lo generado ni el historial |
| `neoplat-windows-1.15.zip` | el `ngplat.exe` y su LEEME |
...
escribir ninguna orden— pregunta qué quieres hacer (crear un juego, abrir el
editor de uno que ya exista o compilarlo) y espera a que pulses Enter antes de
cerrar la ventana. Construirlo desde Linux es
