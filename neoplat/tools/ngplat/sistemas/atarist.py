...
def _graficos_c(build: Build, banco: gfx_st.BancoSt) -> str:
    ...
    partes = [
        ...
        " *",
        " * El `aligned(4)` no es adorno: np_video.c copia estos bytes de cuatro",
        " * en cuatro, y en el 68000 leer una palabra larga de una direccion impar",
        " * es un address error que cuelga la maquina. Como el enlazador coloca los",
        " * arrays de bytes donde le cabe, sin esto la maquina arrancaba o no segun",
        " * cuantos dibujos llevara el juego. */",
        "const uint8_t np_tile_data[NP_TILE_COUNT * %d] __attribute__((aligned(4))) = {"
        ...
        "const uint8_t np_tile_mask[NP_TILE_COUNT * %d] __attribute__((aligned(4))) = {"
        % gfx_st.BYTES_MASCARA,
